...
IO.popen("today.rb", &:read) if Time.now - File.mtime(data_file) > 60
...
# p data
KEYS=%i[id current_price price price_change position hold new_amount old_amount amount_change]
d=[]
data[KEYS].to_a.map do |h|
  d << [h[:id], *h.values_at(*%i[current_price price price_change position hold new_amount old_amount amount_change]).map(&:commify)]
end
